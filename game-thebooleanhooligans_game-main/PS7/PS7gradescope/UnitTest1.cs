...
namespace NetworkUtilTests
{
    ...
    public class NetworkUtilTests
    {
        ...
        // Suppressing the warning here because it's easier than re-designing the tests.
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
        ...
        {
            TcpListener listener = NCSol.NetworkingSol.StartServer(OnNewClient, port);
            return listener;
        }
        ...
        private TcpListener SetupSingleServerTest()
        {
            ...
            NCSol.NetworkingSol.ConnectToServer(saveClientState, "localhost", 2112);
        ...
        private void ReceiveLoop(SocketState state)
        {
            if (state.ErrorOccurred)
                return;
            NCSol.NetworkingSol.GetData(state);
        }

        private void StartReceiveLoop()
        {
            testRemoteSocketState.OnNetworkAction = ReceiveLoop;
            NCSol.NetworkingSol.GetData(testRemoteSocketState);
        }
        ...
        public void TestReceiveTinyMessage(bool clientSide)
        {
            ...
            NCSol.NetworkingSol.Send(testRemoteSocketState.TheSocket, "a");
        ...
        public void TestReceiveNewlines(bool clientSide)
        {
            ...
            NCSol.NetworkingSol.Send(testRemoteSocketState.TheSocket, message);
            Networking.GetData(testLocalSocketState);
        ...
        public void TestRapidReceive(bool clientSide)
        {
            ...
            {
                char c = (char)('a' + (i % 26));
                NCSol.NetworkingSol.Send(testRemoteSocketState.TheSocket, "" + c);
            }
        ...
        public void TestReceiveRemovesPartial(bool clientSide)
        {
            ...
            {
                NCSol.NetworkingSol.Send(testRemoteSocketState.TheSocket, toSend);
            }
        ...
        public void TestReceiveBufferSize(bool clientSide)
        {
            ...
            NCSol.NetworkingSol.Send(testRemoteSocketState.TheSocket, message.ToString());
        ...
        public void TestTwoServers()
        {
            ...
            bool receiveClient1Called = false;
            bool receiveClient2Called = false;
            bool receiveServer1Called = false;
            bool receiveServer2Called = false;
            ...
            NCSol.NetworkingSol.ConnectToServer(Client2OnConnect, "localhost", 2222);
            ...
            NCSol.NetworkingSol.GetData(client2);

            // Send from client1 to server1
            NCSol.NetworkingSol.Send(testRemoteSocketState.TheSocket, "a");

            // Send from client2 to server2
            NCSol.NetworkingSol.Send(client2.TheSocket, "b");
            ...
            WaitForOrTimeout(() => server2Connected && receiveClient1Called && receiveClient2Called && receiveServer1Called && receiveServer2Called, timeout);

            Assert.IsTrue(server2Connected);
            Assert.IsTrue(receiveClient1Called);
            Assert.IsTrue(receiveClient2Called);
            Assert.IsTrue(receiveServer1Called);
            Assert.IsTrue(receiveServer2Called);
        ...
        public void TestSimulateGame()
        {
            ...
            Assert.IsNotNull(clientSockets);
